...
# Local modules
from gsc.utils import copy_graph, graph_prime, graph_power
...
def prime_qudit_LC(graph, node, a, copy=True):
    ...
    for u, v in neigh_k_edges:
        ...
        if lc_graph.has_edge(u, v):
            ...
            new_weight = (uv_weight + a * nv_weight * nu_weight) % prime
            lc_graph[u][v]['weight'] = new_weight
        else:
            weight = (a * nv_weight * nu_weight) % prime
...
def queued_orbit_search(init_graph, local_ops, save_edges, verbose):
    ...
    class_graph.add_node(0, nx_graph=init_graph, edges=init_edges, hash=init_hash)
    ...
    while queue:
        ...
        for rep_node, equiv_nodes in node_equivs.items():
            for op_label, local_op in local_ops:
                ...
                if sorted(new_graph.edges(data='weight')) == sorted(graph.edges(data='weight')):
                    continue
                ...
                try:
                    ...
                    if save_edges:
                        ...
                        if not class_graph.has_edge(graph_label, old_label):
                            class_graph.add_edge(graph_label, old_label, equivs=[equiv_nodes], ops=[op_label])

                        # Else adds any new local ops to edge label
                        elif op_label not in class_graph[graph_label][old_label]['ops']:
                            class_graph[graph_label][old_label]['ops'].append(op_label)
                            class_graph[graph_label][old_label]['equivs'].append(equiv_nodes)
                    continue
                # If not in class, creates new class graph node
                except KeyError:
                    new_label = max(class_graph.nodes()) + 1
                    class_graph.add_node(new_label, nx_graph=new_graph, edges=new_edges, hash=new_hash)
                    if save_edges:
                        class_graph.add_edge(graph_label, new_label, equivs=[equiv_nodes], ops=[op_label])
...
def explore_lc_orbit(init_graph, save_edges=True, verbose=True):
    ...
    prime = graph_prime(init_graph)
    power = graph_power(init_graph)
    if power == 1 and not nx.is_connected(init_graph):
    ...
    if power > 1:
        ...
        local_ops = [
            ('CC%d(c,%d)' % (a, t), make_pp_CC_a(a, t))
            for a in range(1, prime) for t in range(power)
        ]
        local_ops += [
            ('EM%d' % (b), make_EM_b(b))
    ...
    elif prime > 2 and power == 1:
        local_ops = [('LC' + str(a), make_LC_a(a)) for a in range(1, prime)]
        local_ops += [('EM' + str(b), make_EM_b(b)) for b in range(2, prime)]

    else:
        local_ops = [('LC', qubit_LC)]

    # Performs orbit search
    class_graph = queued_orbit_search(init_graph, local_ops, save_edges, verbose)

    # Adds weighted edge data for qudit class graphs
    for _, data in class_graph.nodes.data():
        graph = data['nx_graph']
        edges = data['edges']
...
def get_min_edge_reps(class_graph):
    ...
    min_edges = min(len(graph['edges']) for graph in class_graph.nodes.values())
    min_edge_reps = {
        key: graph
        for key, graph in class_graph.nodes.items()
...
def get_max_edge_reps(class_graph):
    """ Returns all minimum edge representations for a given LC orbit """
    max_edges = max(len(graph['edges']) for graph in class_graph.nodes.values())
    max_edge_reps = {
        key: graph
        for key, graph in class_graph.nodes.items()
...
def export_class_graph(class_graph, filename, min_edge_reps=False):
    ...
    for node, _ in class_graph.nodes.items():
